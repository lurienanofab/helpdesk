...
abstract class DataPlugin{
    ...
    protected function where($criteria = null){
        ...
        $ticket_id = db_input($this->getval("ticket_id", 0, $criteria));
        $ticketID = db_input($this->getval("ticketID", 0, $criteria));
        $resourceId = db_input($this->getval("resource_id", 0, $criteria));
        $assignedTo = db_input($this->getval("assigned_to", "", $criteria));
        $unassigned = db_input($this->getval("unassigned", "", $criteria));
        $email = db_input($this->getval("email", "", $criteria));
        $name = db_input($this->getval("name", "", $criteria));
        $status = db_input($this->getval("status", "", $criteria));
        $sdate = db_input($this->getval("sdate", "", $criteria));
        $edate = db_input($this->getval("edate", "", $criteria));
        $priorityDesc = db_input($this->getval("priority_desc", "", $criteria));
        $result = "";
        $and = " WHERE";
