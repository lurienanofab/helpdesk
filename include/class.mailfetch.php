...
class MailFetcher {
    ...
    function createTicket($mid,$emailid=0){
        ...
        if(preg_match ("[[#][0-9]{1,10}]",$var['subject'],$regs)) {
            ...
            //Allow mismatched emails?? For now YES. [modified by jg on 2013-03-27]. We must allow this because users default scheduler email are used to create tickets but they might use a different email when replying to messages from the ticket system.
			if(false && (!$ticket || strcasecmp($ticket->getEmail(),$var['email'])))
                $ticket=null;
        }
