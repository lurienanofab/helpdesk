...
$sortOptions=LNF::getSortOptions($status);
...
// [2016-06-22 jg] This sets the default sort (order_by). Adding assigned staff so that unassigned tickets are at the top of the list per Tony's request.
$order_by =$order_by?$order_by:'IFNULL(closed_staff.lastname,staff.lastname),IFNULL(closed_staff.firstname,staff.firstname),priority_urgency,effective_date DESC ,ticket.created';
...
LNF::customizeQueryForTicketListBeforeTotal($qselect, $qfrom, $qwhere, $qstr, $search, $startTime, $endTime, $status);
...
LNF::customizeQueryForTicketListAfterTotal($qselect, $qfrom, $qwhere, $qstr, $search, $startTime, $endTime, $status);
...
<div id='advance' style="display:<?=$basic_display?'none':'block'?>">
 ...
 <input type="hidden" name="a" value="search">
  <table>
	...
	<?LNF::printToolSelectForTicketList();?>
    <div>
        Date Span:
    ...
    <tr><td>
       <table width="100%" border="0" cellspacing=0 cellpadding=2 class="dtable" align="center">
        <tr>
	        ...
	        <th width="80">
                <a href="tickets.php?sort=date&order=<?=$negorder?><?=$qstr?>" title="Sort By Date <?=$negorder?>"><?=(strtolower($status)=='closed')?'Closed':'Created'?></a></th>
	        <th width="280">Subject</th>
	        <th width="100">
                <a href="tickets.php?sort=dept&order=<?=$negorder?><?=$qstr?>" title="Sort By Category <?=$negorder?>">Department</a></th>
            <th width="100">
                <a href="tickets.php?sort=resource&order=<?=$negorder?><?=$qstr?>" title="Sort By Resource <?=$negorder?>">Resource</a></th>
            <th width="100">
                <a href="tickets.php?sort=staff&order=<?=$negorder?><?=$qstr?>" title="Sort By Assigned <?=$negorder?>">Assigned</a></th>
	        <th width="70">
        ...
        if($tickets_res && ($num=db_num_rows($tickets_res))):
            ...
            <tr class="<?=$class?> " id="<?=$row['ticket_id']?>">
                ...
                <td align="center" nowrap><?=LNF::getDateForTicket($row, $status)?></td>
                ...
                <td><?=LNF::getResourceIdForTicket($row)?></td>
                <td><?=LNF::getStaffNameForTicket($row, $status)?></td>
            ...
            <tr class="<?=$class?>"><td colspan="9"><b>Query returned 0 results.</b></td></tr>
        <?
        endif; ?>
       </table>
    </td></tr>
