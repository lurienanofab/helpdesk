...
include_once(INCLUDE_DIR.'class.pushnotification.php');
...
class Ticket{
    ...
    function assignStaff($staffId,$message,$alertstaff=true) {
        ...
        if($this->setStaffId($staff->getId())){
            ...
            if($alertstaff && ($thisuser && $staff->getId()!=$thisuser->getId())) { //No alerts for self assigned.
                ...
                //[jg 2016-01-19] push notification (assign staff)
                $pn = new PushNotification();
                $pn->execute(array(
                    'user'	=> $staff->getUserName(),
                    'subj' 	=> 'Assigned to ticket',
                    'msg'	=> 'You have been assigned to ticket #'.$this->getExtId(),
                    'url'	=> '{host}/helpdesk/scp/tickets.php?id='.$this->getId()
                ));
            ...
            LNF::sendClientEmailOnAssign($this);
            return true;
        }
        return false;
    }
    ...
    function postMessage($msg,$source='',$msgid=NULL,$headers='',$newticket=false){
        ...
        $sql='INSERT INTO '.TICKET_MESSAGE_TABLE.' SET created=NOW() '.
			 ...
			 ',message='.db_input($msg). //we want users to be able to enter < and >. The input must be stored raw, so all text must be put through htmlspecialchars before it is output to the browser
                    ...
                    if(($resp=db_query($sql)) && db_num_rows($resp) && list($subj,$body)=db_fetch_row($resp)){
                        ...
                        if($email && $email->getId()) {
                            ...
                            foreach( $recipients as $k=>$staff){
                                ...
                                //[jg 2016-01-19] push notification (post message)
                                $pn = new PushNotification();
                                $pn->execute(array(
                                    'user'	=> $staff->getUserName(),
                                    'subj' 	=> 'New message posted',
                                    'msg'	=> $subj,
                                    'url'	=> '{host}/helpdesk/scp/tickets.php?id='.$this->getId()
                                ));
    ...
    function postResponse($msgid,$response,$signature='none',$attachment=false,$canalert=true){
        ...
        $sql= 'INSERT INTO '.TICKET_RESPONSE_TABLE.' SET created=NOW() '.
				...
				',response='.db_input($response). //we want users to be able to enter < and >. The input must be stored raw, so all text must be put through htmlspecialchars before it is output to the browser
    ...
    function create($var,&$errors,$origin,$autorespond=true,$alertstaff=true) {
       ...
       /* Coders never code so fully and joyfully as when they do it for free  - Peter Rotich */
        ...
        $sql=   'INSERT INTO '.TICKET_TABLE.' SET created=NOW() '.
                ...
                ',email='.db_input($var['email']).
                ',name='.db_input(Format::striptags($var['name'])).
                ',subject='.db_input(Format::striptags(substr($var['subject'], 0, 255))).
            ...
            if($alertstaff && $cfg->alertONNewTicket() && is_object($ticket)){
                ...
                if(($resp=db_query($sql)) && db_num_rows($resp) && list($subj,$body)=db_fetch_row($resp)){
                    ...
                    if($email && $email->getId()) {
                        ...
                        LNF::addGroupMemberRecipients($recipients, $dept);

                        // Sending mulitple individual emails was causing problems because there are a lot of recipients.
                        // To work around this send a single email with all the recipients added
                        ...
                        foreach( $recipients as $k=>$staff){
                            ...
                            //[jg 2016-01-19] push notification (new ticket)
                            /*$pn = new PushNotification();
                            $pn->execute(array(
                                'user'  => $staff->getUserName(),
                                'subj'  => 'New ticket created',
                                'msg'   => $subj,
                                'url'   => '{host}/helpdesk/scp/tickets.php?id='.$ticket->getId()
                            ));*/
                        ...
                        if ($sendSingleEmail && count($sentlist) > 0){
                            $alert = str_replace("%staff","Staff",$body);
                            $email->send(implode(",", $sentlist),$subj,$alert);
                        }
                    }
                }else {
